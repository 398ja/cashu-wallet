# Secure Coding Guidelines for cashu-wallet

This document outlines the mandatory secure coding practices for the `cashu-wallet` project. These guidelines are derived from industry best practices (OWASP, Oracle, etc.) and must be followed for all contributions.
