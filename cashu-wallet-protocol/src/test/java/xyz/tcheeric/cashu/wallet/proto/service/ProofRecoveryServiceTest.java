...
class ProofRecoveryServiceTest {
    ...
    /**
     * A real compressed point on secp256k1: the generator G.
     *
     * <p>This used to be 0x02 followed by 32 zero bytes, which has the right length and prefix but
     * is not a point on the curve. `PublicKey` only checked length and prefix, so the fixture was
     * accepted and failed later, deep in the crypto, as an `IllegalStateException`. cashu-lib
     * 0.30.0 verifies the point at construction (audit L-4), so the fixture is now rejected up
     * front, which is the check working rather than a regression.
     */
